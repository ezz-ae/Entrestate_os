...
const BYPASS_HEADER =
  process.env.SMOKE_BYPASS_HEADER
  ?? process.env.VERCEL_BYPASS_HEADER
...
async function hit(
  ...
  try {
    ...
    const expectsJson = options?.expectJson ?? true
    if (expectsJson && !contentType.includes("application/json")) {
      return { ok: false, ms, error: `Expected JSON, got ${contentType || "unknown"}` }
    }
...
const SMOKE_TESTS: SmokeTest[] = [
  ...
  // ── Healthcheck ──────────────────────────────────────────────────
  {
    ...
    }, { accept: "text/html", expectJson: false }),
  },
