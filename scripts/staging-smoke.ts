 ...
 * Entrestate Terminal — staging smoke runner (13 checks)
 *
 * Where this came from: this file was found at `lib/timetable/smoke (7).ts`,
 * a browser download that had been dropped into a source directory and never
 * wired up. Nothing imported it, so nothing ever ran it. It is kept because it
 * checks things `scripts/post-deploy-smoke.ts` does not — the chat API's
 * response shape, PII leakage on /api/embed, evidence provenance, HTTP method
 * enforcement, and graceful handling of an empty body.
 *
 * It is deliberately NOT wired into `pnpm smoke` or into CI: it has not been
 * run green against this deployment yet, and a smoke gate nobody has watched
 * pass is worse than no gate. Run it by hand first:
 *
 *   pnpm smoke:staging                        # against STAGING_URL
 *   pnpm smoke:staging -- --url https://...   # against a preview
 *   pnpm smoke:staging -- --prod              # against PRODUCTION_URL
 *
 * Env vars:
 *   STAGING_URL          e.g. https://entrestate-staging.vercel.app
 *   PRODUCTION_URL       e.g. https://terminal.entrestate.com
 *   VERCEL_BYPASS_TOKEN  Vercel protection bypass secret (staging only)
 *   SMOKE_TIMEOUT_MS     Per-request timeout (default: 8000)
 *
 * In CI: set VERCEL_BYPASS_TOKEN as a GitHub secret. Never commit the token.
 */
...
const BASE_URL =
  args.url ??
  (args.prod
    ? process.env.PRODUCTION_URL ?? "https://terminal.entrestate.com"
